...
// Simplified utility for proportional sizing within the sidebar
const vw = (value) => `${value}vw`;
const vh = (value) => `${value}vh`;

const navItems = [
  { name: 'Dashboard', icon: LayoutGrid, isActive: false },
  { name: 'Products', icon: Box, isActive: false },
  { name: 'Alerts', icon: Bell, isActive: true },
  { name: 'Reports', icon: FileText, isActive: false },
  { name: 'Settings', icon: Settings, isActive: false },
];

// Component for a single navigation item
const NavItem = ({ name, icon: Icon, isActive }) => {
  ...
  return (
    <div
      className={`flex items-center cursor-pointer transition-all ${activeClass}`}
      // Apply proportional padding and margin for consistent spacing
      style={{
        padding: `${vh(1.8)} ${vw(1.5)}`,
        fontSize: vw(1.1),
        borderRadius: isActive ? `0 ${vw(0.3)} ${vw(0.3)} 0` : '0', // Small radius on right for active state
        marginBottom: vh(0.5)
      }}
    >
      <Icon 
...
const SideBar = () => {
  return (
    <div 
      ...
      style={{ width: '15vw' }} // Dimension kept as requested
    >
